...
const MISSIONS = [
  ...
  { title: 'Festival des Sans-Papiers', desc: 'Événement annuel de visibilisation et de revendication culturelle, prévu en septembre 2026 à Bruxelles.', icon: Globe, color: 'border-[#0F0F0F]' },
];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {KEY_FIGURES.map((fig, i) => (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] shadow-soft-elegant border border-gray-100 flex flex-col items-center text-center">
              <div className={`p-3 rounded-2xl bg-gray-50 mb-4 ${fig.color}`}><fig.icon className="h-6 w-6" /></div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-16">
        <div className="bg-white border border-gray-100 rounded-[3rem] p-10 md:p-14 shadow-soft-elegant mb-16">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <div className="inline-block bg-guinea-yellow/20 text-[#0F0F0F] px-4 py-1 rounded-full font-black text-[10px] uppercase tracking-widest mb-6 border border-guinea-yellow/30">Fiche institutionnelle</div>
              ...
              <div className="space-y-4 text-sm font-medium text-gray-700">
                <div className="flex items-start gap-3"><Award className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" /><div><span className="font-black text-[#0F0F0F]">Forme juridique : </span>Association Sans But Lucratif (ASBL), droit belge</div></div>
                <div className="flex items-start gap-3"><Building className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" /><div><span className="font-black text-[#0F0F0F]">Numéro BCE : </span>{PRESS_CONTACTS.bce}</div></div>
                <div className="flex items-start gap-3"><MapPin className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" /><div><span className="font-black text-[#0F0F0F]">Siège social : </span>{PRESS_CONTACTS.address}</div></div>
                <div className="flex items-start gap-3"><Calendar className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" /><div><span className="font-black text-[#0F0F0F]">Active depuis : </span>2022 — 3 ans d'opérations continues</div></div>
                <div className="flex items-start gap-3"><Globe className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" /><div><span className="font-black text-[#0F0F0F]">Zone d'action : </span>Région de Bruxelles-Capitale</div></div>
              </div>
            </div>
            <div className="bg-[#0F0F0F] text-white rounded-[2rem] p-10">
              <div className="flex items-center gap-3 mb-8"><Newspaper className="h-6 w-6 text-guinea-yellow" /><span className="font-black text-[10px] uppercase tracking-widest text-guinea-yellow">Contact unique</span></div>
              <p className="text-gray-300 font-medium mb-8 leading-relaxed">Toutes les demandes presse, interviews et demandes de partenariat sont traitées via un point de contact unique.</p>
              <div className="space-y-4">
                <button onClick={handleCopyEmail} className="w-full flex items-center justify-between bg-white/10 hover:bg-white/20 transition-all p-4 rounded-2xl group">
        ...
        {activeTab === 'press' && (
          <div className="space-y-10 animate-in fade-in duration-300">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {MISSIONS.map((m, i) => (
                  <div key={i} className={`bg-white p-8 rounded-[2rem] shadow-soft-elegant border-l-8 ${m.color}`}>
                    <div className="flex items-center gap-4 mb-4"><m.icon className="h-6 w-6 text-gray-400" /><h3 className="text-xl font-black text-[#0F0F0F]">{m.title}</h3></div>
            ...
            <div className="bg-guinea-red text-white rounded-[2rem] p-10">
              <div className="flex items-center gap-3 mb-8"><FileText className="h-6 w-6 text-guinea-yellow" /><h2 className="text-2xl font-serif font-black">Éléments de langage</h2></div>
              <div className="space-y-4">
                {['Ballal ASBL accompagne 130 personnes sans papiers à Bruxelles.',"Fondée en 2022, l'association intervient sur le logement d'urgence, l'alimentation et les droits.",'Ballal est une ASBL de droit belge (BCE 1016.925.333), active en Région de Bruxelles-Capitale.',"L'association organise le Festival des Sans-Papiers, événement annuel prévu en septembre 2026."].map((point, i) => (
                  <div key={i} className="flex items-start gap-4"><CheckCircle className="h-5 w-5 text-guinea-yellow flex-shrink-0 mt-0.5" /><p className="font-medium leading-relaxed">{point}</p></div>
                ))}
              </div>
            </div>
        ...
        {activeTab === 'partners' && (
          <div className="space-y-10 animate-in fade-in duration-300">
            ...
            <div className="bg-[#0F0F0F] text-white rounded-[2rem] p-10">
              <div className="flex items-center gap-3 mb-8"><Target className="h-6 w-6 text-guinea-yellow" /><h2 className="text-2xl font-serif font-black">Ce que Ballal peut apporter</h2></div>
              <div className="grid md:grid-cols-2 gap-6">
                {[{titre:'Ancrage territorial',desc:'Présence directe à Molenbeek et réseau de confiance avec les communautés.'},{titre:'Légitimité opérationnelle',desc:"3 ans d'actions concrètes, 130 personnes suivies, ASBL enregistrée."},{titre:'Réseau de terrain',desc:'Liens avec les squats, occupations solidaires et associations partenaires bruxelloises.'},{titre:'Visibilité militante',desc:'Festival, présence médiatique en construction — audience engagée.'}].map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    ...
                    <div><div className="font-black text-white mb-1">{item.titre}</div><div className="text-sm text-gray-400 font-medium">{item.desc}</div></div>
                  </div>
                ))}
              </div>
            </div>
