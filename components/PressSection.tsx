...
const KEY_FIGURES = [
  ...
  { value: '1', label: 'Permanence hebdomadaire', sub: 'accueil sur rendez-vous à Bruxelles', icon: Building, color: 'text-earth-black' },
];
...
const PressSection: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-soft-paper pb-24">
      ...
      <div className="bg-earth-black text-white py-20 border-b-8 border-guinea-yellow relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {KEY_FIGURES.map((fig, i) => (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] shadow-soft-elegant border border-gray-100 flex flex-col items-center text-center">
              ...
              <div className="text-4xl font-serif font-black text-earth-black mb-1">{fig.value}</div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-16">
        ...
        <div className="bg-white border-4 border-earth-black rounded-[3rem] p-10 md:p-14 shadow-brutal mb-16">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              <div className="inline-block bg-guinea-yellow/20 text-earth-black px-4 py-1 rounded-full font-black text-[10px] uppercase tracking-widest mb-6 border border-guinea-yellow/30">
                Fiche institutionnelle
              </div>
              <h2 className="text-4xl font-serif font-black text-earth-black mb-6">Ballal ASBL</h2>
              <div className="space-y-4 text-sm font-medium text-gray-700">
                <div className="flex items-start gap-3">
                  ...
                  <div><span className="font-black text-earth-black">Forme juridique : </span>Association Sans But Lucratif (ASBL), droit belge</div>
                </div>
                <div className="flex items-start gap-3">
                  <Building className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" />
                  <div><span className="font-black text-earth-black">Numéro BCE : </span>{PRESS_CONTACTS.bce}</div>
                </div>
                <div className="flex items-start gap-3">
                  <MapPin className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" />
                  <div><span className="font-black text-earth-black">Siège social : </span>{PRESS_CONTACTS.address}</div>
                </div>
                <div className="flex items-start gap-3">
                  <Calendar className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" />
                  <div><span className="font-black text-earth-black">Active depuis : </span>2022 — 3 ans d'opérations continues</div>
                </div>
                <div className="flex items-start gap-3">
                  <Globe className="h-4 w-4 text-guinea-red mt-1 flex-shrink-0" />
                  <div><span className="font-black text-earth-black">Zone d'action : </span>Région de Bruxelles-Capitale</div>
                </div>
              </div>
            </div>

            <div className="bg-earth-black text-white rounded-[2rem] p-10">
              <div className="flex items-center gap-3 mb-8">
        ...
        <div className="flex gap-4 mb-10">
          {(['press', 'partners'] as const).map((tab) => (
            <button
              ...
              className={`px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-earth-black text-white shadow-brutal' : 'bg-white text-gray-400 border border-gray-200 hover:text-earth-black'}`}
            >
        ...
        {activeTab === 'press' && (
          <div className="space-y-10 animate-in fade-in duration-300">
            <div>
              <h2 className="text-3xl font-serif font-black text-earth-black mb-8">Nos missions</h2>
                ...
                {MISSIONS.map((m, i) => (
                  <div key={i} className={`bg-white p-8 rounded-[2rem] shadow-soft-elegant border-l-8 ${m.color}`}>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <h3 className="text-xl font-black text-earth-black">{m.title}</h3>
                    </div>
            ...
            <div className="bg-white border-4 border-earth-black rounded-[2rem] p-8 flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                <h3 className="text-2xl font-serif font-black text-earth-black mb-2">Kit presse</h3>
              ...
              <a href="mailto:admin@ballal.be?subject=Demande kit presse Ballal ASBL" className="flex-shrink-0 bg-earth-black text-white px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-guinea-red transition-all flex items-center gap-3 shadow-brutal">
        ...
        {activeTab === 'partners' && (
          <div className="space-y-10 animate-in fade-in duration-300">
            <div className="grid md:grid-cols-3 gap-6">
              {PARTNERSHIP_TYPES.map((type, i) => (
                <div key={i} className="bg-white p-8 rounded-[2rem] shadow-soft-elegant border border-gray-100">
                  <h3 className="text-lg font-black text-earth-black mb-6 pb-4 border-b border-gray-100">{type.title}</h3>
            ...
            <div className="bg-white border-4 border-guinea-green rounded-[2rem] p-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                <h3 className="text-2xl font-serif font-black text-earth-black mb-2">Proposer un partenariat</h3>
