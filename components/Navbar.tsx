    ...
    <div className="fixed top-0 w-full z-[150]">
      <div className="bg-guinea-red text-white py-2 px-6 flex justify-between items-center border-b border-white/10">
        <div className="flex items-center gap-3">
          <ShieldAlert className="h-3 w-3 text-guinea-yellow" aria-hidden="true" />
          <span className="text-[9px] font-bold uppercase tracking-[0.2em]">Réseau Ballal Belgique</span>
        </div>
        <div className="flex items-center gap-4">
          <a
            href="tel:0493434383"
            className="font-bold text-[9px] hover:text-guinea-yellow transition-colors tracking-widest"
            aria-label="Numéro d'urgence : 0493 43 43 83"
          >
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={logout}
                className="text-[9px] font-bold underline hover:text-guinea-yellow focus:outline-none focus:ring-1 focus:ring-guinea-yellow rounded"
              >
                DÉCONNEXION
              </button>
            </div>
            ...
            <button
              onClick={onOpenAuth}
              className="flex items-center gap-1 text-[9px] font-bold hover:text-guinea-yellow transition-colors focus:outline-none focus:ring-1 focus:ring-guinea-yellow rounded"
            >
              <LogIn className="h-3 w-3" aria-hidden="true" /> ACCÈS MEMBRE
            </button>
          )}
        </div>
      </div>

      <nav aria-label="Navigation principale" className="px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center bg-white/95 backdrop-blur-md px-6 py-3 rounded-2xl shadow-soft-elegant border border-gray-100">
          ...
          >
            <div className="p-2 bg-guinea-green rounded-xl group-hover:rotate-6 transition-transform" aria-hidden="true">
          ...
          <div className="hidden lg:flex gap-1" role="list">
          ...
          <div className="lg:hidden">
            <button
              className="p-2 bg-earth-black text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-earth-black focus:ring-offset-1"
        ...
        {isMobileMenuOpen && (
          <div
            id="mobile-menu"
            className="lg:hidden mt-4 bg-white rounded-2xl shadow-2xl p-4 border border-gray-100 animate-in slide-in-from-top-4"
          >
              ...
              {MAIN_NAV_ITEMS.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl font-bold text-xs uppercase tracking-widest ${
                    isActive(item.path) ? 'bg-guinea-red text-white' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
